...
test('generateLayout', function(t) {
    spritezero.generateLayout({ imgs: getFixtures(), pixelRatio: 1, unique: false }, function(err, layout) {
        t.ifError(err);
...
test('generateLayout bench (concurrency=1,x10)', function(t) {
    var start = +new Date();
    var q = queue(1);
    for (var i = 0; i < 10; i++) q.defer(spritezero.generateLayout, { imgs: getFixtures(), pixelRatio: 1, unique: false });
...
test('generateLayout bench (concurrency=4,x20)', function(t) {
    var start = +new Date();
    var q = queue(4);
    for (var i = 0; i < 20; i++) q.defer(spritezero.generateLayout, { imgs: getFixtures(), pixelRatio: 1, unique: false });
    q.awaitAll(function(err) {
        t.ifError(err);
        t.ok(true, (+new Date() - start) + 'ms');
        t.end();
...
test('generateLayoutUnique', function(t) {
    spritezero.generateLayoutUnique({ imgs: getFixtures(), pixelRatio: 1, unique: false }, function(err, layout) {
        t.ifError(err);
...
test('generateLayout', function(t) {
    spritezero.generateLayout({ imgs: getFixtures(), pixelRatio: 1, unique: true }, function(err, formatted) {
        t.ifError(err);
...
test('generateLayoutUnique', function(t) {
    spritezero.generateLayoutUnique({ imgs: getFixtures(), pixelRatio: 1, unique: true }, function(err, formatted) {
        t.ifError(err);
...
test('generateImage', function(t) {
    [1, 2, 4].forEach(function(scale) {
        t.test('@' + scale, function(tt) {
            ...
            spritezero.generateLayout({ imgs: getFixtures(), pixelRatio: scale, unique: true }, function(err, formatted) {
                tt.ifError(err);
                spritezero.generateLayout({ imgs: getFixtures(), pixelRatio: scale, unique: false }, function(err, layout) {
                    tt.ifError(err);
...
test('generateImageUnique', function(t) {
    [1, 2, 4].forEach(function(scale) {
        t.test('@' + scale, function(tt) {
            ...
            spritezero.generateLayoutUnique({ imgs: getFixtures(), pixelRatio: scale, unique: true }, function(err, formatted) {
                tt.ifError(err);
                spritezero.generateLayoutUnique({ imgs: getFixtures(), pixelRatio: scale, unique: false }, function(err, layout) {
                    tt.ifError(err);
                    if (update) fs.writeFileSync(jsonPath, stringify(formatted, { space: '  ' }));
                    tt.deepEqual(formatted, JSON.parse(fs.readFileSync(jsonPath)));
...
test('generateLayoutUnique with empty input', function(t) {
    spritezero.generateLayoutUnique({ imgs: [], pixelRatio: 1, unique: true }, function(err, layout) {
        t.ifError(err);
        t.deepEqual(layout, {});
        t.end();
    });
...
test('generateImage with empty input', function(t) {
    spritezero.generateLayout({ imgs: [], pixelRatio: 1, unique: false }, function(err, layout) {
        t.ifError(err);
...
test('generateImage unique with empty input', function(t) {
    spritezero.generateLayoutUnique({ imgs: [], pixelRatio: 1, unique: false }, function(err, layout) {
        t.ifError(err);
        spritezero.generateImage(layout, function(err, sprite) {
            t.notOk(err, 'no error');
            t.ok(sprite, 'produces image');
