...
const renameMapboxMetadata = {
    ...
    fn: function(item) {
        if (item.hasAttr('id')) {
            ...
            if (id.startsWith(metadataPrefix)) {
                item.removeAttr('id');
                item.addAttr({
                    name: 'mapbox:metadata',
                    value: id.substring(metadataPrefix.length),
                    prefix: '',
...
function extractMetadata(img, callback) {
    try {
        let svg = Buffer.isBuffer(img.svg) ? img.svg.toString('utf-8') : img.svg;

        let result = parseSvg(svg);
            ...
            try {
                ...
                result = invokePlugins(result, info, preparePlugins);

                // We'll walk through the plugins that collapse groups until there are no changes to the output.
                do {
                    previousSVG = svg;
                    result = invokePlugins(result, info, collapsePlugins);
                ...
                result = invokePlugins(result, info, extractPlugins);
            } catch (err) {
                return callback(err);
            }
