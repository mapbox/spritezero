...
function strongRound(value, precision) {
  ...
  if (+value.toFixed(precision) !== value) {
    const rounded = +value.toFixed(precision - 1);
    value =
      +Math.abs(rounded - value).toFixed(precision + 1) >= error
  ...
  fn: function (item, params, info) {
    if (
      ...
      if (metadata.value.startsWith('stretch-x')) {
        (info.metadata.stretchX = info.metadata.stretchX || []).push([
          bounds.left,
          bounds.right
        ]);
      } else if (metadata.value.startsWith('stretch-y')) {
        (info.metadata.stretchY = info.metadata.stretchY || []).push([
          bounds.top,
          bounds.bottom
        ]);
        ...
        metadata.value === 'text-placeholder'
      ) {
...
const collapseConfig = createConfig({
  full: true,
  plugins: ['convertTransform', 'moveGroupAttrsToElems', 'collapseGroups']
});
