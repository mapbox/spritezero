 ...
 * @param   {boolean}               [options.removeOversizedIcons] optional, if set, filters out icons that mapnik says are too big
 * @param   {boolean}               [options.stretchMetadata]      optional, defaults to true; if set to false, omits stretch metadata (`content`, `stretchX`, `stretchY`) in {@link DataLayout}
 * @param   {number}                [options.concurrency]          optional, defaults to undefined, which means concurrency is unbounded
 * @param   {Function}              callback                       Accepts two arguments, `err` and `layout` Object
 * @return  {DataLayout|ImgLayout}  layout                         Generated Layout Object with sprite contents
 */
...
function generateLayoutInternal(options, callback) {
    ...
    var q = new queue(options.concurrency);
 ...
 * @param  {number}    [options.mode] Defaults to alpha preserving 'hextree'. But if you don't need alpha, `octree` might be faster
 * @param  {number}    [options.compression] Defaults to -1 which means node-mapnik will select the image encoding's default
 * @param  {Function}    callback  Accepts two arguments, `err` and `image` data
 */
function generateOptimizedImage(layout, options, callback) {
    ...
    mapnik.blend(layout.items, {
        ...
        mode: options.mode || 'hextree',
        compression: options.compression || -1, // -1 triggers default fallback in node-mapnik
        width: layout.width,
        height: layout.height
    }, callback);
}
