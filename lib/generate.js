...
function generateLayout(options, callback) {
  ...
  options.extractMetadata =
    options.extractMetadata === undefined ? true : options.extractMetadata;
...
function generateLayoutInternal(options, callback) {
  ...
  if (options.unique) {
    ...
    var svgPerItemId = {}; // eslint-disable-line no-var

    /* The items for each SVG signature */
    var itemIdsPerSvg = {}; // eslint-disable-line no-var

    options.imgs.forEach((item) => {
      const svg = item.svg.toString('base64');
  ...
  function createImages(img, callback) {
    const mapnikOpts = { scale: options.pixelRatio };
    ...
    mapnik.Image.fromSVGBytes(img.svg, mapnikOpts, (err, image) => {
      ...
      if (
        err &&
        err.message.match(
          /image created from svg must have a width and height greater (then|than) zero/
        )
      )
        return callback(null, null);
      ...
      if (
        options.extractMetadata &&
        options.format &&
        (img.svg.includes('mapbox-stretch') ||
          img.svg.includes('mapbox-text-placeholder'))
      ) {
        ...
        extractMetadata(metaOps, (err, metadataProps) => {
          return callback(err, {
            ...
            height: image.height(),
            ...metadataProps,
            ...{ h: image.height(), w: image.width() } // potpack need w and h fields.
          });
        });
      } else {
      ...
      imagesWithSizes.forEach((item) => {
        ...
        itemIdsToUpdate.forEach((itemIdToUpdate) => {
          ...
          ['content', 'placeholder', 'stretchX', 'stretchY'].forEach((key) => {
            if (item[key]) {
              dataLayout[itemIdToUpdate][key] = item[key];
            }
          });
        });
      });
