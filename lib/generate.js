...
var sdf = require('@elastic/fontnik').pathToSDF;
...
function generateLayoutInternal(options, callback) {
    ...
    function sdfRender(img, dimensions, callback) {
      var buffer = 3;
      var cutoff = 2/8;

      var svg = JSDOM.fragment(img.svg.toString());

      var w = dimensions.width * options.pixelRatio;
      var h = dimensions.height * options.pixelRatio;
      var wb = w + 2 * buffer;
      var hb = h + 2 * buffer;
